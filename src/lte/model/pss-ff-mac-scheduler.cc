...
{
    NS_LOG_FUNCTION(this << " RNTI " << params.m_rnti << " txMode " << +params.m_transmissionMode);
...
{
    NS_LOG_FUNCTION(this << params.m_rnti << +params.m_logicalChannelIdentity);
    ...
    const FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    for (std::size_t i = 0; i < m_dlInfoListBuffered.size(); i++)
    {
        ...
        if (retx.at(0) || retx.at(1))
        {
            ...
            NS_LOG_INFO(this << " HARQ retx RNTI " << rnti << " harqId " << +harqId);
            ...
            for (std::size_t j = 0; j < nLayers; j++)
            {
                if (retx.at(j))
                {
                    if (j >= dci.m_ndi.size())
                    {
                        ...
                        NS_LOG_INFO(this << " layer " << +j << " no txed (MIMO transition)");
                    }
                    else
                    {
                        dci.m_ndi.at(j) = 0;
                        ...
                        NS_LOG_INFO(this << " layer " << +j << " RV " << +dci.m_rv.at(j));
                    }
                }
                else
                {
                    ...
                    NS_LOG_INFO(this << " layer " << +j << " no retx");
                }
            }
            for (std::size_t k = 0; k < (*itRlcPdu).second.at(0).at(dci.m_harqProcess).size(); k++)
            {
                ...
                for (std::size_t j = 0; j < nLayers; j++)
                {
                    if (retx.at(j))
                    {
                        if (j < dci.m_ndi.size())
                        {
                            NS_LOG_INFO(" layer " << +j << " tb size " << dci.m_tbsSize.at(j));
            ...
            {
                NS_FATAL_ERROR("Unable to find HARQ timer for RNTI " << +rnti);
            }
    ...
    { // has data in RLC buffer
        ...
        for (auto it = ueSet.begin(); it != ueSet.end(); it++)
        {
            ...
            if ((itRnti != rntiAllocated.end()) || (!HarqProcessAvailability((*it).first)))
            {
                ...
                if (itRnti != rntiAllocated.end())
                {
                    NS_LOG_DEBUG(this << " RNTI discarded for HARQ tx" << +(*it).first);
                }
                if (!HarqProcessAvailability((*it).first))
                {
                    NS_LOG_DEBUG(this << " RNTI discarded for HARQ id" << +(*it).first);
                }
                continue;
            }
                    ...
                    for (auto it = tdUeSet.begin(); it != tdUeSet.end(); it++)
                    {
                             ...
                             0)) // CQI == 0 means "out of range" (see table 7.2.3-1 of 36.213)
                        {
                            for (uint8_t k = 0; k < nLayer; k++)
                            {
                                ...
                                colMetric += static_cast<double>(sbCqi) / (*itSbCqiSum).second;
                            }
                        }
    ...
    while (itMap != allocationMap.end())
    {
        ...
        if (lcActives == 0)
        {
            ...
            lcActives = static_cast<uint16_t>(65535); // UINT16_MAX;
        }
        ...
        for (uint8_t j = 0; j < nLayer; j++)
        {
            NS_LOG_INFO(this << " Layer " << +j << " CQI selected " << +worstCqi.at(j));
        }
        ...
        for (uint8_t j = 0; j < nLayer; j++)
        {
            ...
            NS_LOG_INFO(this << " Layer " << +j << " MCS selected"
        ...
        for (auto itBufReq = m_rlcBufferReq.begin(); itBufReq != m_rlcBufferReq.end(); itBufReq++)
        {
                 ...
                 ((*itBufReq).second.m_rlcStatusPduSize > 0)))
            {
                ...
                for (uint8_t j = 0; j < nLayer; j++)
                {
                    ...
                    NS_LOG_INFO(this << " LCID " << +newRlcEl.m_logicalChannelIdentity << " size "
                                     << newRlcEl.m_size << " layer " << +j);
        ...
        if (m_harqOn)
        {
            ...
            if (itHarqTimer == m_dlHarqProcessesTimer.end())
            {
                NS_FATAL_ERROR("Unable to find HARQ timer for RNTI " << +newEl.m_rnti);
            }
    ...
    for (auto itStats = m_flowStatsDl.begin(); itStats != m_flowStatsDl.end(); itStats++)
    {
        ...
        if (itUeScheduleted != tdUeSet.end())
        {
            (*itStats).second.secondLastAveragedThroughput =
                ...
                ((1 / m_timeWindow) *
                 static_cast<double>((*itStats).second.lastTtiBytesTransmitted) / 0.001);
        }
        ...
        (*itStats).second.lastAveragedThroughput =
            ...
            ((1.0 / m_timeWindow) * static_cast<double>((*itStats).second.lastTtiBytesTransmitted) /
             0.001);
        (*itStats).second.lastTtiBytesTransmitted = 0;
    }
    ...
    const FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
    ...
    for (unsigned int i = 0; i < params.m_cqiList.size(); i++)
    {
        ...
        {
            NS_LOG_LOGIC("wideband CQI " << +params.m_cqiList.at(i).m_wbCqi.at(0) << " reported");
    ...
    const FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
    ...
    if (m_harqOn)
    {
        ...
        for (std::size_t i = 0; i < params.m_ulInfoList.size(); i++)
        {
            if (params.m_ulInfoList.at(i).m_receptionStatus == UlInfoListElement_s::NotOk)
            {
                ...
                uint8_t harqId =
                    static_cast<uint8_t>((*itProcId).second - HARQ_PERIOD) % HARQ_PROC_NUM;
                NS_LOG_INFO(this << " UL-HARQ retx RNTI " << rnti << " harqId " << +harqId << " i "
                ...
                if (free)
                {
                    ...
                    NS_LOG_INFO(this << " Send retx in the same RBs " << +dci.m_rbStart << " to "
    ...
    do
    {
        ...
        NS_LOG_INFO(this << " UE Allocation RNTI " << (*it).first << " startPRB "
                         << +uldci.m_rbStart << " nPRB " << +uldci.m_rbLen << " CQI " << cqi
                         << " MCS " << +uldci.m_mcs << " TBsize " << uldci.m_tbSize << " RbAlloc "
                         << rbAllocated << " harqId " << +harqId);
...
PssFfMacScheduler::RefreshDlCqiMaps()
{
    ...
    {
        NS_LOG_INFO(this << " P10-CQI for user " << (*itP10).first << " is " << +(*itP10).second
                         << " thr " << +m_cqiTimersThreshold);
        if ((*itP10).second == 0)
        {
    ...
    {
        NS_LOG_INFO(this << " A30-CQI for user " << (*itA30).first << " is " << +(*itA30).second
                         << " thr " << +m_cqiTimersThreshold);
        if ((*itA30).second == 0)
        {
            // delete correspondent entries
...
PssFfMacScheduler::RefreshUlCqiMaps()
{
    ...
    {
        NS_LOG_INFO(this << " UL-CQI for user " << (*itUl).first << " is " << +(*itUl).second
                         << " thr " << +m_cqiTimersThreshold);
        if ((*itUl).second == 0)
        {
            // delete correspondent entries
...
PssFfMacScheduler::UpdateDlRlcBufferInfo(uint16_t rnti, uint8_t lcid, uint16_t size)
{
    ...
    if (it != m_rlcBufferReq.end())
    {
        NS_LOG_INFO(this << " UE " << rnti << " LC " << +lcid << " txqueue "
...
{
    NS_LOG_FUNCTION(this << " RNTI " << rnti << " txMode " << +txMode);
