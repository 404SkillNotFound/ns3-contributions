...
TdBetFfMacScheduler::RefreshDlCqiMaps()
{
    ...
    while (itP10 != m_p10CqiTimers.end())
    {
        NS_LOG_INFO(this << " P10-CQI for user " << (*itP10).first << " is "
                         << static_cast<uint32_t>((*itP10).second) << " thr "
                         << static_cast<uint32_t>(m_cqiTimersThreshold));
        if ((*itP10).second == 0)
        {
    ...
    while (itA30 != m_a30CqiTimers.end())
    {
        NS_LOG_INFO(this << " A30-CQI for user " << (*itA30).first << " is "
                         << static_cast<uint32_t>((*itA30).second) << " thr "
                         << static_cast<uint32_t>(m_cqiTimersThreshold));
        if ((*itA30).second == 0)
        {
            // delete correspondent entries
...
TdBetFfMacScheduler::RefreshUlCqiMaps()
{
    ...
    while (itUl != m_ueCqiTimers.end())
    {
        NS_LOG_INFO(this << " UL-CQI for user " << (*itUl).first << " is "
                         << static_cast<uint32_t>((*itUl).second) << " thr "
                         << static_cast<uint32_t>(m_cqiTimersThreshold));
        if ((*itUl).second == 0)
        {
            // delete correspondent entries
