    ...
    const FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
    ...
    NS_LOG_INFO(this << " RNTI " << params.m_rnti << " LC " << +params.m_logicalChannelIdentity
                     << " RLC tx size " << params.m_rlcTransmissionQueueSize << " RLC retx size "
    ...
    const FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
    ...
    for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++)
    {
...

        {
            NS_LOG_LOGIC(this << " User " << (*it).m_rnti << " LC "
                              << +(*it).m_logicalChannelIdentity << " is active, status  "
    ...
    if (m_nextRntiDl != 0)
    {
        NS_LOG_DEBUG("Start from the successive of " << +m_nextRntiDl);
        for (it = m_rlcBufferReq.begin(); it != m_rlcBufferReq.end(); it++)
        {
    ...
    do
    {
                ...
                ((*it).m_rlcRetransmissionQueueSize > 0) || ((*it).m_rlcStatusPduSize > 0))
            {
                ...
                for (uint8_t j = 0; j < nLayer; j++)
                {
                    ...
                    NS_LOG_INFO(this << "LCID " << +newRlcEl.m_logicalChannelIdentity << " size "
                                     << rlcPduSize << " ID " << (*it).m_rnti << " layer " << +j);
        ...
        uint32_t rbgMask = 0;
        uint16_t i = 0;
        NS_LOG_INFO(this << " DL - Allocate user " << newEl.m_rnti << " LCs " << +(*itLcRnti).second
                         << " bytes " << tbSize << " mcs " << +newDci.m_mcs.at(0) << " harqId "
                         << +newDci.m_harqProcess << " layers " << nLayer);
        NS_LOG_INFO("RBG:");
        while (i < rbgPerTb)
        {
    ...
    const FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
        ...
        for (std::size_t i = 0; i < params.m_ulInfoList.size(); i++)
        {
            if (params.m_ulInfoList.at(i).m_receptionStatus == UlInfoListElement_s::NotOk)
            {
                ...
                if (free)
                {
                    ...
                    NS_LOG_INFO(this << " Send retx in the same RBGs " << +dci.m_rbStart << " to "
    ...
    do
    {
        ...
        NS_LOG_INFO(this << " UL Allocation - UE " << (*it).first << " startPRB "
                         ...
                         << " MCS " << +uldci.m_mcs << " TBsize " << uldci.m_tbSize << " harqId "
                         << +harqId);
